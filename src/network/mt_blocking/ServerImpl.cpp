...
void ServerImpl::Stop() {
    ...
    std::lock_guard<std::mutex> l_g(_m);
    std::for_each(_socket_set.cbegin(), _socket_set.cend(),[](int client_socket){
...
void ServerImpl::Join() {
    ...
    while(!_socket_set.empty() && !running){
        _cv.wait(l);
    }
}

void ServerImpl::client_function(int client_socket) {
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {  //make unique lock within this scope
            std::unique_lock<std::mutex> l(_m);
